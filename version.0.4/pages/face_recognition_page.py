...
def getStudentNameAndID(course_id):
    name_list = []
    id_list = []
    with open('version.0.4/datasets/data/student_in_course_detail/'+ course_id +'.csv',  encoding="utf8", mode='r', newline='') as csv_file:
...
def writeAttendance(course_id, student_id, name):
    with open('version.0.4/datasets/data/attendance/'+ course_id +'_attendence.csv', encoding="utf8", mode='a', newline='') as csv_file:
...
def runFaceRecognition(course_id):
    ...
    model_path = 'version.0.4/training_faces.yml'
